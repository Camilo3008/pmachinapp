...
export const UserUpdateForm = ({ userData }) => {
  ...
  const [mutateError, serMutateError] = useState({});

  const { user, refetch: recargarUsuarioLoged } = useAuth();
  ...
  const { refetch: recargaUsuarios } = useFetchDataUser();
  ...
  const { mutateAsync: actualizarInfo, isPending } = useMutation({
    mutationFn: async (dataUsuario) => {
      // http://127.0.0.1:8000/api/user/{id}/
      return await axiosClient.put(`user/user/${Usuario.id}/`, dataUsuario);
    },
  });
  ...
  const handleEditUsuario = async (data) => {
    ...
    try {
      await actualizarInfo(data);

      if (user.fk_rol.nombre === "administrador") {
        await recargaUsuarios();
        navigation("/panel-control");
        toast.success("Usuario Actualizado con exito");
        return;
      }
      recargarUsuarioLoged();
      navigation("/perfil");
    } catch (error) {
      if (error.response && error.response.data) {
        serMutateError(error.response.data);
      }
    }
  };
            ...
            <div className="p-7">
              ...
              <div className="mb-5.5 flex flex-col gap-10 sm:flex-row">
                <InputForm
                  ...
                  name={"username"}
                />
                {mutateError.username && <>{mutateError.username}</>}
              </div>
              ...
              <div className="w-full sm:w-1/2">
                <InputForm
                  ...
                  name={"email"}
                />
                {mutateError.email && <>{mutateError.email}</>}
              </div>
